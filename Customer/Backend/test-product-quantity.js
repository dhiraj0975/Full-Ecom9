...
async function testProductQuantityDeduction() {
  try {
    ...
    const initialResponse = await axios.get(import.meta.env.VITE_API_URL +`/api/products/${productId}`);
    ...
    const updatedResponse = await axios.get(import.meta.env.VITE_API_URL +`/api/products/${productId}`);
